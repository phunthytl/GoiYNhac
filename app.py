...
LANGUAGE_LABELS = {
    ...
    "nl": "Dutch",
    "sv": "Swedish",
    "unknown": "Không xác định / Không quan trọng",
}
...
def detailed_genres(songs: pd.DataFrame) -> list[str]:
    detailed = set()
    for col in ["genres_titles_text", "genres_all_titles_text"]:
        if col in songs.columns:
            for text in songs[col].fillna(""):
                detailed.update(split_genre_text(text))
    return sorted(detailed - set(top_level_genres(songs)))


def genre_children_by_parent(songs: pd.DataFrame) -> dict[str, list[str]]:
    """Lấy sub-genre theo genre cha từ fma_metadata/genres.csv, chỉ giữ genre có xuất hiện trong dữ liệu hiện tại."""
...
def profile():
    ...
    # Merge on song_id, rename songs.id to song_id để tránh conflict
